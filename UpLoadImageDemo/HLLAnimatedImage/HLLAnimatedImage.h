...
extern const NSTimeInterval kHLLAnimatedImageDelayTimeIntervalMinimum;

//
//  An `HLLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `HLLAnimatedImageView`.
...
//
@interface HLLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `HLLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `HLLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface HLLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [HLLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
