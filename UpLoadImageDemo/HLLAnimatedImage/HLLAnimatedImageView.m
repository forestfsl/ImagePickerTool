...
@protocol HLLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(HLLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface HLLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<HLLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation HLLAnimatedImageView
...
- (void)setAnimatedImage:(HLLAnimatedImage *)animatedImage
{
...
{
    // Presision is set to half of the `kHLLAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kHLLAnimatedImageDelayTimeIntervalMinimum;
