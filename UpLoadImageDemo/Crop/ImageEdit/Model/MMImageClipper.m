...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _fillColor = [UIColor colorWithRed:255.0 green:255.0 blue:255.0 alpha:0.4];
        _pointWidth = 30.f;
    }
    return self;
}
...
{
    UIView *pointView = [[UIView alloc] initWithFrame:CGRectMake(point.x - _pointWidth/2, point.y - _pointWidth/2, _pointWidth, _pointWidth)];
...
- (NSArray *)getCGPointValues
{
    ...
    for (NSInteger i = 0; i < self.points.count; i ++) {
        ...
        CGPoint point = CGPointMake(pointView.frame.origin.x + _pointWidth/2, pointView.frame.origin.y + _pointWidth/2);
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextMoveToPoint(context, firstPoint.frame.origin.x +_pointWidth/2, firstPoint.frame.origin.y +_pointWidth/2);
    for (uint i = 1; i < self.points.count; i ++) {
        UIView *point = [self.points objectAtIndex:i];
        CGContextAddLineToPoint(context, point.frame.origin.x + _pointWidth/2, point.frame.origin.y + _pointWidth/2);
    }
...
{
    CGPoint locationPoint = [[touches anyObject] locationInView:self];
    if (CGRectContainsPoint(CGRectMake(_pointWidth/2, _pointWidth/2, self.al_width-_pointWidth, self.al_height-_pointWidth), locationPoint) == NO) {
        return;
    }
    if (self.activePoint) {
        self.activePoint.frame = CGRectMake(locationPoint.x -_pointWidth/2, locationPoint.y -_pointWidth/2, _pointWidth, _pointWidth);
