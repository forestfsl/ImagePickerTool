...
#import "HLLModifyCamera+Helper.h"



@interface HLLModifyCamera () <AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, copy) void (^didRecordCompletionBlock)(HLLModifyCamera *camera, NSURL *outputFileUrl, NSError *error);
@end

NSString *const HLLModifyCameraErrorDomain = @"HLLModifyCameraErrorDomain";

@implementation HLLModifyCamera
...
{
    return [self initWithQuality:AVCaptureSessionPresetHigh position:HLLCameraPositionRear videoEnabled:videoEnabled];
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        [self setupWithQuality:AVCaptureSessionPresetHigh
                      position:HLLCameraPositionRear
                  videoEnabled:YES];
    }
    return self;
}
...
- (void)setupWithQuality:(NSString *)quality
                position:(HLLCameraPosition)position
            videoEnabled:(BOOL)videoEnabled
{
    ...
    _flash = HLLCameraFlashOff;
    _mirror = HLLCameraMirrorAuto;
...
- (void)start
{
    [HLLModifyCamera requestCameraPermission:^(BOOL granted) {
        if(granted) {
            // request microphone permission if video is enabled
            if(self.videoEnabled) {
                [HLLModifyCamera requestMicrophonePermission:^(BOOL granted) {
                    if(granted) {
                        [self initialize];
                    }
                    else {
                        NSError *error = [NSError errorWithDomain:HLLModifyCameraErrorDomain
                                                             code:HLLModifyCameraErrorCodeMicrophonePermission
                                                         userInfo:nil];
                        [self passError:error];
                    }
                }];
            ...
            NSError *error = [NSError errorWithDomain:HLLModifyCameraErrorDomain
                                                 code:HLLModifyCameraErrorCodeCameraPermission
                                             userInfo:nil];
            [self passError:error];
        }
    }];
...
- (void)initialize
{
    if(!_session) {
        ...
        switch (self.position) {
            case HLLCameraPositionRear:
                if([self.class isRearCameraAvailable]) {
                    devicePosition = AVCaptureDevicePositionBack;
                } else {
                    devicePosition = AVCaptureDevicePositionFront;
                    _position = HLLCameraPositionFront;
                }
                break;
            case HLLCameraPositionFront:
                if([self.class isFrontCameraAvailable]) {
                    devicePosition = AVCaptureDevicePositionFront;
                } else {
                    devicePosition = AVCaptureDevicePositionBack;
                    _position = HLLCameraPositionRear;
                }
                break;
            default:
...
-(void)capture:(void (^)(HLLModifyCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage animationBlock:(void (^)(AVCaptureVideoPreviewLayer *))animationBlock
{
    if(!self.session) {
        NSError *error = [NSError errorWithDomain:HLLModifyCameraErrorDomain
                                    code:HLLModifyCameraErrorCodeSession
                                userInfo:nil];
...
- (void)startRecordingWithOutputUrl:(NSURL *)url didRecord:(void (^)(HLLModifyCamera *camera, NSURL *outputFileUrl, NSError *error))completionBlock
{
    ...
    if(!self.videoEnabled) {
        NSError *error = [NSError errorWithDomain:HLLModifyCameraErrorDomain
                                             code:HLLModifyCameraErrorCodeVideoNotEnabled
                                         userInfo:nil];
        [self passError:error];
        return;
    }
    
    if(self.flash == HLLCameraFlashOn) {
...
- (BOOL)updateFlashMode:(HLLCameraFlash)cameraFlash
{
    ...
    if(cameraFlash == HLLCameraFlashOn) {
        flashMode = AVCaptureFlashModeOn;
    } else if(cameraFlash == HLLCameraFlashAuto) {
...
- (void)setMirror:(HLLCameraMirror)mirror
{
    _mirror = mirror;
    ...
    switch (mirror) {
        ...
        case HLLCameraMirrorAuto: {
            BOOL shouldMirror = (_position == HLLCameraPositionFront);
            if ([videoConnection isVideoMirroringSupported]) {
...
- (HLLCameraPosition)togglePosition
{
    ...
    if(self.position == HLLCameraPositionRear) {
        self.cameraPosition = HLLCameraPositionFront;
    } else {
        self.cameraPosition = HLLCameraPositionRear;
    }
    
    return self.position;
}

- (void)setCameraPosition:(HLLCameraPosition)cameraPosition
{
    if(_position == cameraPosition || !self.session) {
        return;
    }
    
    if(cameraPosition == HLLCameraPositionRear && ![self.class isRearCameraAvailable]) {
        return;
    }
    
    if(cameraPosition == HLLCameraPositionFront && ![self.class isFrontCameraAvailable]) {
        return;
    }
