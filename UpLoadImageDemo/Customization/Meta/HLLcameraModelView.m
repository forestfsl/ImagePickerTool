...
- (void)setupView {
    ...
    self.labelContainerView.al_centerY += 8.0f;
...
- (void)switchMode:(UISwipeGestureRecognizer *)recognizer {
    if (recognizer.direction == UISwipeGestureRecognizerDirectionLeft && !self.maxLeft) {
        ...
        [UIView animateWithDuration:0.28
                         ...
                         animations:^{
                             self.labelContainerView.al_x -= 62;
...
- (void)setCameraMode:(HLLCameraMode)cameraMode {
    if (_cameraMode != cameraMode) {
        _cameraMode = cameraMode;
        if (cameraMode == THCameraModePhoto) {
            self.captureButton.selected = NO;
            self.captureButton.captureButtonMode = HLLCaptureButtonModePhoto;
            self.layer.backgroundColor = [UIColor blackColor].CGColor;
        } else {
            self.captureButton.captureButtonMode = HLLCaptureButtonModeVideo;
...
- (void)layoutSubviews {
    ...
    self.labelContainerView.al_x = CGRectGetMidX(self.bounds) - (self.videoStringWidth / 2.0);
}
